...
- **项目级** `.claude/settings.json`：`Edit`、`Write`、`mcp__codex-reviewer__codex` matcher（iSparto 工作流规则——代码直写拦截、Codex prompt 规范）。`/init-project` 和 `/migrate` 注册，`/start-working` 自动校验补全。
