...
One framework-side rule refinement candidate surfaced from a conversational exchange during Wave B close-out. The insight (BLOCKING marker rule is coarse and over-fires on any Tier 1 modification regardless of semantic impact on next Wave) is a legitimate framework-improvement opportunity that the user's critical-thinking push produced organically during the `/end-working` flow. Not blocking any current work. Recorded here + in Wave B plan.md entry + in session-log.md as next-`/plan` carry-over.
