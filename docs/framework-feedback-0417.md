...
## Rule 6 — `/end-working` session-log Wave-count lacks a mechanical verification step

**Rule ID:** `commands/end-working.md` Step 4 (Generate a session report and append it to `docs/session-log.md`) — Notes section authoring guidance.

**Gap description:** When a Wave's session-log Notes section references an aggregate Wave count (e.g., "N Waves accumulated since the v0.7.X tag" as part of a release-candidate framing), Step 4 does not require Lead to compute the count mechanically via `git log --oneline --no-merges <last-release-tag>..HEAD` or `git log --oneline main..HEAD | grep -c 'Merge pull request'` before writing the prose. This parallels Rule 2 (commit-count verification) but applies to the human-prose Wave count that appears in the Notes section. Observed failure mode in v0.7.8 Framework Polish Wave's session-log entry (2026-04-17): "7 Waves accumulated since the v0.7.7 tag" was written without mechanical verification — the accurate count is 6 (Wave A / Wave B / Semantic Gate / Gate Narrowing / Wave C including Rule 2 as T4 of the same Wave / v0.7.8 Polish). The miscount propagated to the next `/end-working` briefing before Lead caught and corrected it in the release-prep summary. Release artifact (CHANGELOG, tag, GitHub Release) was produced with the correct count (6), so no downstream user impact — but the session-log entry on main still carries the "7" prose, and future framework-feedback audits would find the inconsistency.

**Expected behavior:** Add a sub-bullet to `commands/end-working.md` Step 4 Notes-section authoring guidance parallel to Rule 2's commit-count verification: "If the Notes section references an aggregate Wave count since the last release tag, compute it mechanically via `git log --oneline main.. | grep -c 'Merge pull request'` (or `git log --oneline <last-tag>.. | grep -c 'Merge pull request'` when writing before release) — not by mental recall. Co-located Waves (e.g., a Wave that completes several internal tasks as one plan.md entry + one PR) count as 1, not N." The mechanical step is cheap (one shell command) and catches the Wave-count class of errors at authoring time rather than surfacing them to the next `/start-working` briefing.

**Session context:** v0.7.8 Release close-out (2026-04-17, same session as this entry). PO audit classified G2 as PASS (awareness) with explicit note that "the `/end-working` command template currently has no mechanical step that computes the Wave count from `git log`." The release artifact is clean; the audit-trail inconsistency (one session-log Notes line says "7", everything else says "6") is preserved as evidence that Lead self-corrected mid-session. Recommendation: fold into the next framework-polish Wave alongside Rules 3, 4, 5 (all deferred to the same cluster). Low priority — the class of error is rare (aggregate Wave-count phrasing appears only in release-candidate Waves that reference a prior release tag), high-cost to fix (command template edit + downstream CLAUDE-TEMPLATE.md mirror + possibly plan-template.md update).

## Summary

Six framework-side rule refinement candidates surfaced across 2026-04-17's Wave sequence. Rule 1 (BLOCKING marker rule over-coarse) was surfaced during Wave B close-out and **addressed by the BLOCKING Marker Semantic Gate Wave + Gate Narrowing Wave in the same day** — now retrospective documentation of the problem and fix. Rule 2 (plan.md commit count timing ambiguity) surfaced from the Semantic Gate Wave's PO audit and was **addressed by the Wave C + Rule 2 Wave in the same day (CLAUDE.md + CLAUDE-TEMPLATE.md wording updated)** — also now retrospective. Rule 3 (BLOCKING literal sentinel vs rationale prose write-together rule) surfaced from Wave C's PO audit and is deferred to the next framework-polish Wave or v0.8 roadmap planning session. Rule 4 (IR skip at Wave boundary lacks explicit exit criteria) and Rule 5 (BLOCKING marker boundary acknowledgement path undefined for mid-session CLAUDE.md updates) surfaced from v0.7.8 Framework Polish Wave's PO audit (both classified PASS under spirit of rule, framework-level gaps) and are deferred to the same follow-up Wave. Rule 6 (Wave-count mechanical verification missing from `/end-working` Step 4 Notes-section authoring) surfaced from v0.7.8 Release close-out's PO audit (classified PASS with awareness; release artifact is clean, session-log has one line with the incorrect "7" count and the self-correction preserved as audit trail). None of the six rules — addressed or open — is blocking any current work.
