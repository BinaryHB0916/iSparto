...
Developer (Codex) reviews technical architecture (Team Lead invokes via MCP, based on tech-spec.md):
  ...
  - Whether tech choices match requirements
    |
Independent Reviewer (Codex CLI in tmux pane — GPT-5.5, cross-provider isolation on top of zero inherited context; most token-intensive role per invocation; reads product-spec then tech-spec independently):
...
> **Model selection:** Tier 1 implementation steps use the Developer default model (gpt-5.5, xhigh); QA steps and Tier 2 use gpt-5.5-mini (high). See docs/configuration.md "Developer tiered model strategy" for details.
