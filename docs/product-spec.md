...
- **Process Observer 合规监督**: 两层机制——Hooks 实时拦截灾难性操作（不可逆/数据丢失/敏感信息泄露），事后审计对照行为准则检查 session 全流程合规性（13 项 checklist）。Observer 不参与开发决策，只监督；偏差报告输出到 session briefing，不自动修改文件；修正建议反馈到规则中形成自我改进闭环
