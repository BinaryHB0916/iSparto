...
| Not sure if `/migrate` is safe for my project | Worried about unintended changes to an existing codebase | Run `/migrate --dry-run` first — it scans the project and shows the full migration plan without making any changes. Review the plan, then run `/migrate` when ready |
