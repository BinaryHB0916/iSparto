...
| 项 | 要求 | 说明 |
|----|------|------|
| 平台 | macOS | Agent Team 模式依赖 iTerm2 内建的 tmux 集成 |
...
| Git | 任意版本 | 版本控制 |
| 终端 | iTerm2 | Agent Team 的 tmux 模式需要 iTerm2 内建集成，无需单独安装 tmux |
...
Lead 自动完成 commit + push + Doc Engineer 审计 + Process Observer 审计 + 简报输出。如果当前 Wave 全部任务都完成了，会自动创建 PR 并合并到 main；如果还在 Wave 中途，只推送不合并。
