...
| FR-39 | A-layer Peer Review 整套机制铲除——18 天 dogfood 期间没产生实际价值 + 用户明示 "去掉"。删 `docs/design-principles/a-layer-peer-review.md` 整文件 + 同步清理 `agents/independent-reviewer.md`(L8/L14/L108-110)、`CLAUDE.md`(L101 那句 + L124 Documentation Index 行)、`CLAUDE-TEMPLATE.md`、`docs/concepts.md` Quick Reference 行(19→18)、`commands/plan.md` L32 spawn 调用、`docs/design-decisions.md` 相关行、`docs/design-principles/information-layering-policy.md` 中"subject to Peer Review"语句 | 单独 docs/ 分支 Wave，markdown-only，按自指 boundary Lead direct edit；CHANGELOG `[Unreleased] ### Removed` 加一条；ship as v0.8.5 | high | User direction 2026-04-27 — "去掉，不是 wire 进去"；Lead 之前误读 design intent 把 v0.7.4 spec 当成现仍持有的需求并推荐 (A) wire 全 commands，被用户否决 |
| FR-40 | `hooks/process-observer/scripts/pre-tool-check.sh` Edit/Write 分支 (L286-322) 只 basename + 扩展名 match，不做 path-context 检查；`.sh` 在 `workflow-rules.json:8` 无条件 allowed。CLAUDE.md L52 + CLAUDE-TEMPLATE.md L89-91 明文写"用户项目通过 CLAUDE-TEMPLATE.md 继承不继承自指例外"，但 hook 这一层没法 enforce 这个区别——意味着用户 fork iSparto 后，`/init-project` + `/migrate` 把同一份 hook 装到用户项目，他的 `src/run.sh` 也会被允许 Lead 直接 Edit；规则 vs. enforcement 之间有 gap | `pre-tool-check.sh` Edit/Write 分支加 path-context 检查：iSparto self-referential paths (`hooks/`, `scripts/`, `lib/`, `commands/`, `agents/`, `templates/`, `docs/` 加 root-level Tier 1 文件) 允许 `.sh` 直编辑；其他路径 `.sh` 走 Codex。需要 Codex 实现 + Lead 复审。 | major | Codex CONFIRMED MAJOR via codex exec 2026-04-27；evidence: pre-tool-check.sh L286-322 + workflow-rules.json:8 + CLAUDE-TEMPLATE.md L89-91 + commands/init-project.md L33-40 + commands/migrate.md L53-60 |
| FR-41 | `docs/plan.md` L205 Tech Ecosystem Tracking 行 "GPT-5.4 退役 / GPT-6 发布" 触发条件已被 gpt-5.5 release 满足 (2026-04-27 之前 OpenAI 发了 gpt-5.5 + gpt-5.5-pro，Codex CLI 的 `[notice.model_migrations]` 已 auto-bump) | 该行要么改成跟踪 gpt-5.5 退役 / GPT-6 发布的下一代触发，要么直接删行（事件已发生，跟踪不再有意义） | low | v0.8.4 spec sync session 2026-04-27 surfaced |
| FR-42 | CLAUDE.md Branch Protocol 允许 `feat/ fix/ hotfix/ docs/ release/`，但 `commands/end-working.md` Step 4 引入的 governance-maintenance session 类型语义上是 `chore/`——`chore/` 不在 approved 列表里。今天 v0.8.4 spec sync session 我先建 `chore/` 后 rename 成 `docs/` 才符合规则 | 二选一：(a) CLAUDE.md Branch Protocol 加 `chore/` 进 approved set 给 governance-maintenance 用；或 (b) `commands/end-working.md` Step 4 governance-maintenance 定义里明文写"用 `docs/` 前缀"。同步 `agents/process-observer-audit.md` A1 row 接受这个新分支前缀 | low | v0.8.4 spec sync session 2026-04-27 — Lead 实地踩到的 prefix vs. session-type 命名 gap |
| FR-43 | `commands/end-working.md:34` cross-reference 写"CLAUDE.md Solo/Agent Team workflow step 4"，但实际 emergency-hotfix exception 定义在 `docs/collaboration-mode.md` Step 4 + `docs/workflow.md` Hotfix Workflow + `CLAUDE-TEMPLATE.md` L71/L83。指针漂移导致 audit agent 搜不到定义 | `commands/end-working.md:34` cross-reference 改成 "docs/collaboration-mode.md Solo/Agent Team workflow step 4 + docs/workflow.md Hotfix Workflow" | low | 2026-04-27 audit Agent 2 false-positive — 报"exception undefined"，原因是搜了 collab-mode.md 但 cross-reference 指向 CLAUDE.md，路径漂移 |
| FR-44 | `commands/end-working.md` Step 9 写"DE pre-merge gate 3-iteration loop bound 不能 skip"，但 PR template L148 给"Doc Engineer audit: sub-agent run ✅ / Lead self-assessed ✅"——两者矛盾。Step 9 没 sanction "Lead self-assessed" 路径，但 PR template 暗示该路径合法（emergency-hotfix substitute path 才是合法的 Lead self-assessed 路径，但 Step 9 没 cite 它） | 二选一：(a) `commands/end-working.md` Step 9 加显式 Lead self-assessed 路径条件，cite docs/workflow.md Hotfix Workflow 的 emergency-hotfix substitute path；或 (b) PR template L148 删 "Lead self-assessed ✅" 选项。 | major | 2026-04-27 audit Agent 2 finding F6 |
| FR-45 | `docs/workflow.md` Wave-level safety-net carve-out 措辞 "markdown-only / data-only / no Tier 1 logic changes" 容易被误读成 "Tier 1 shell scripts 也算自指例外"。Lead 在 v0.8.2 hotfix 误读这一条直接改了 `install.sh`，绕过了 Codex review；用户当场拦下来纠错 + v0.8.3 重做时让 Codex 实现 + Lead 复审 | `docs/workflow.md` Wave-level safety-net carve-out 段加加粗显式排除 "shell logic changes are NOT covered by the markdown/data carve-out — Tier 1 shell still requires Codex review" | medium | 2026-04-27 v0.8.2 hotfix 自查 + 用户拦截 — Lead Implementation Protocol 误读事件直接证据 |
