...
- [ ] 自定义角色-模型绑定 — 每个角色（Lead / Developer / Reviewer / Doc Engineer）可绑定不同的 AI 模型/服务，不再硬编码。**前置判断：只有当生态开放到用户真的需要混用不同模型时才做；如果大家仍然用最强模型组合，通用配置无必要。最可能的场景是 Anthropic + OpenAI 两家模型混用，不太可能扩展到其他厂商**
