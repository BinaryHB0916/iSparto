...
Historical Wave narratives for v0.6.x / v0.7.x live in `docs/session-log.md` — see the "已完成 Wave 索引" table below for reverse-chronological anchors. Current active phase: v0.9.x — cross-provider repositioning shipped, CI gate in place; the v0.8.0 observation period closed with the v0.8.0 release (tracker below retained as history); the v0.8 external-user validation gate remains open (deprioritized by user direction 2026-06-10).

🚨 BLOCKING: Next Wave requires NEW SESSION

> Rationale (Cross-provider Repositioning + CI Wave, 2026-06-10): This Wave modified `CLAUDE.md` in three places. Decision aid: (a) Behavior change — `.github/` was added to the self-referential boundary enumeration, changing Lead's direct-edit scope; (b) New identifier — new CI module row (`.github/workflows/ci.yml`) in Module Boundaries; (c) Contract/interface change — Project Overview now states the cross-provider positioning and the Common Commands lint line points to the CI gate instead of "no automation". Marker emitted so the next Lead's `# claudeMd` cache picks up the new boundary scope, the CI module, and the repositioned overview before the next Wave.

> Superseded marker below (v0.9.0, retained as history):

🚨 BLOCKING: Next Wave requires NEW SESSION

> Rationale (v0.9.0 Command Rename Wave): This Wave modified `CLAUDE.md` (Module Boundaries Snapshot Engine row text + multiple Development Rule clauses referencing renamed command files). Decision aid all three "yes": (a) Behavior change — Lead now invokes `/start-isparto`, `/end-isparto`, `/plan-isparto`, etc.; (b) New identifier — 10 new slash command names + new `install.sh --self-test-migration` flag; (c) Contract/interface change — `install.sh --upgrade` path now includes a file-existence-gated stale-file cleanup that snapshots and deletes the 10 pre-v0.9.0 command files on first post-release run. Marker emitted to force a fresh session so the Lead's `# claudeMd` cache picks up the new names + migration logic before any /start-isparto.
...
|------|------|--------------------|
| Cross-provider Repositioning + CI Introduction — positioning switched from "Agent Team framework for Claude Code" to "open-source cross-provider AI dev team, built on Claude Code + Codex CLI" across README/README.zh-CN (new lead + why-two-providers paragraph + comparison-table row + dogfooding-evidence section with render-verified anchors) / docs/product-spec.md / docs/zh/quick-start.md / CLAUDE.md overview / GitHub description+topics / isparto.dev (website PR #6 incl. og-image re-render); GPT-5.5 blind review of the proposal (PROCEED WITH CHANGES) corrected the one-direction overclaim to the bidirectional cross-provider claim. CI introduced via `.github/workflows/ci.yml` (syntax + shellcheck + 7 guardian self-tests on macOS/Ubuntu matrix + repo guardians + installer smoke; first run 6/6 green) with shellcheck remediation 14→0 (`${ISPARTO_HOME:?}` rm guards, bash-3.2-safe snapshot prune rewrite, scoped disables); Codex diff review caught the tmux pre-flight P0 pre-commit. PRs #256 + #257. Why no IR at Wave boundary: non-carve-out skip — FR-19 condition (iii) not met (work-unit PRs were Lead-self-assessed); root cause is FR-47's structural false-positive path (13 CRITICAL missing-tech-spec on iSparto self-review), unfixed at session time; substantive cross-provider coverage came from two GPT-5.5 reviews (positioning blind review PROCEED WITH CHANGES + CI diff review FAIL→P0-fix→green); PO audit F1 recorded the deviation, follow-ups FR-58/FR-59/FR-60. | 2026-06-10 | [session-log](session-log.md#2026-06-10-session--cross-provider-repositioning--ci-introduction) |
| v0.9.0 Command Rename — all 10 iSparto slash commands renamed to `-isparto` suffix (`/start-isparto`, `/end-isparto`, `/plan-isparto`, `/doctor-isparto`, `/init-isparto`, `/migrate-isparto`, `/restore-isparto`, `/release-isparto`, `/security-isparto`, `/env-isparto`) + `install.sh` v0.9.0 file-existence-gated migration block (B.2 cleanup + B.3 `version_lt` + B.4 `--self-test-migration` + B.5 `isparto.sh:160` uninstall help 7→10 names) + `scripts/check-command-rename.sh` dedicated rename guardian (8-fixture self-test PASS) + `docs/troubleshooting.md` "Old commands not found after v0.9.0 rename" English-only migration section + full-repo non-Tier-4 sweep across CLAUDE.md / CLAUDE-TEMPLATE.md / 10 commands/ / 2 agents/ / 7 scripts/ / lib/snapshot.sh / 3 templates/ / 2 hooks/ / 17 docs/ (+ 3 design-principles + 1 zh + 1 observation-period) / README + README.zh-CN + CONTRIBUTING + feature_request issue template. VERSION untouched at 0.8.4; CHANGELOG only `[Unreleased]` (release execution moves to a separate Post-merge Session per the plan). | 2026-05-12 | [session-log](session-log.md#2026-05-12-session-wave-v090-command-rename-isparto-suffix) |
| v0.8.0 Doc Alignment — Phase 3 (README v0.8.0 Discoverability + New-Concept Quick Reference; `README.md` / `README.zh-CN.md` v0.8.0 pointer at L17 + Wave definition at L30 first use + `docs/concepts.md` Quick Reference 16→19 rows for A-layer Peer Review / Guardian Scripts / Observation-period Tracker. Closes the v0.8.0 Doc Alignment arc — FR-35/36/37/38 all migrated to session-log narrative.) | 2026-04-24 | [session-log](session-log.md#2026-04-24-session-c-wave-v080-doc-alignment-phase-3-readme-v080-discoverability-new-concept-quick-reference) |
...
**v0.8 验收条件：**
- [x] 新用户入门修复（README Prerequisites、/init-isparto Next Steps、troubleshooting 安装问题）— PR #78
- [x] 代码关键修复（snapshot 路径编码+向后兼容、release.sh 跨平台、install.sh Python3 检查）— PR #78
- [x] 文档对齐（命令计数、术语统一、Solo Checklist）— PR #78
- [ ] 1 个外部用户冷启动验证（看 README → 安装 → /init-isparto → /start-isparto → /end-isparto 全流程）
...
- [x] Process Observer /end-isparto 集成 — end-working.md 中触发合规审计并输出偏差报告
...
| FR-2 | PR test plan cannot distinguish whether Process Observer ran as independent sub-agent or Lead self-assessed — blocks audit verification of sub-agent independence | `commands/end-isparto.md` Step 4 + PR body template | high | feedback-0405 F2 |
...
| FR-18 | BLOCKING sentinel and rationale lack an explicit "same edit" write-together rule — pre-commit interim state (rationale present, sentinel not yet appended) misclassified as WARN in PO audit | `commands/end-isparto.md` Step 2 BLOCKING decision — add atomic-write requirement | medium | feedback-0417 Rule 3 |
| FR-20 | BLOCKING marker boundary acknowledgement path undefined for the case where CLAUDE.md was modified mid-session and surfaced to Lead via system-reminder injection | `commands/start-isparto.md` Step 0 — add sentence documenting the non-standard acknowledgement path | low | feedback-0417 Rule 5 |
| FR-21 | `/end-isparto` Step 4 Notes-section authoring guidance does not require mechanical computation of aggregate Wave count via `git log` before writing prose (parallels FR-7's commit-count verification) | `commands/end-isparto.md` Step 4 Notes guidance — add sub-bullet requiring mechanical Wave-count verification | low | feedback-0417 Rule 6 |
| FR-22 | BLOCKING marker semantic gate in `commands/end-isparto.md` Step 2 is scoped to Wave-completion entries only ("Wave-completion entries only; skip for mid-Wave updates") and is silent on non-Wave sessions that modify CLAUDE.md with behavioral rule changes — such sessions can add behavior rules without emitting BLOCKING or a skip rationale | `commands/end-isparto.md` Step 2 BLOCKING decision preamble — remove "Wave-completion entries only" qualifier and apply the semantic gate to any session touching CLAUDE.md, with unconditional skip-rationale requirement | medium | PO audit of 2026-04-17 TODO Consolidation session — E4 WARN |
...
| FR-25 | `commands/end-isparto.md` Step 5 (PO audit spawn) runs before Step 3's IR completion is confirmed, placing F1 check in IN-PROGRESS state as the structural norm rather than exception; current F1 spec handles this but step-ordering implications are not documented | `commands/end-isparto.md` Step 5 — document F1 IN-PROGRESS as structural norm for PO audits spawned in this step; Lead should note F1 IN-PROGRESS in B-layer briefing's IR slot rather than flagging as deviation | low | PO audit of 2026-04-20 Wave 2 /doctor session — framework-side rule correction #2 |
...
| FR-39 | A-layer Peer Review 整套机制铲除（用户 direction） | 删 `docs/design-principles/a-layer-peer-review.md` + 清理 7 处 cross-reference (CLAUDE.md / CLAUDE-TEMPLATE.md / agents/independent-reviewer.md / docs/concepts.md / commands/plan-isparto.md / docs/design-decisions.md / information-layering-policy.md) | high | 2026-04-27 user direction |
| FR-40 | `pre-tool-check.sh` Edit/Write 分支只 ext-match 不 path-gate；用户 fork iSparto 后 `.sh` 直编辑权被错误继承 (CLAUDE-TEMPLATE.md 明文不让，hook 无法 enforce) | `pre-tool-check.sh` 加 path-context 检查区分 iSparto self-referential paths vs 用户项目，需 Codex 实现 + Lead 复审 | major | 2026-04-27 Codex confirmed |
| FR-41 | plan.md L205 Tech Ecosystem Tracking 行 "GPT-5.4 退役" 触发条件已被 gpt-5.5 release 满足 | 更新行措辞跟踪下一代触发，或直接删行 | low | 2026-04-27 |
| FR-42 | governance-maintenance session 命名 vs CLAUDE.md Branch Protocol approved 前缀冲突 (`chore/` 不在允许集，但语义合适) | 二选一：approved 集加 `chore/`，或 governance-maintenance 明文用 `docs/` | low | 2026-04-27 |
| FR-43 | `commands/end-isparto.md:34` cross-reference 指向 "CLAUDE.md Solo/Agent Team workflow step 4"，实际定义在 docs/collaboration-mode.md + docs/workflow.md | 修指针指向正确路径 | low | 2026-04-27 |
| FR-44 | `commands/end-isparto.md` Step 9 与 PR template L148 关于 DE "Lead self-assessed" 路径条件不一致 | 二选一对齐：Step 9 cite emergency-hotfix substitute path，或 PR template 删选项 | major | 2026-04-27 |
...
| FR-47 | `agents/independent-reviewer.md` agent prompt 假定 user-project 结构（必有 `docs/product-spec.md` + `docs/tech-spec.md`），跑在 iSparto 自指 review 时缺 tech-spec 直接 BLOCK + 13 条 CRITICAL 都是"missing tech-spec"衍生；CLAUDE.md 已豁免 Codex 强制但 IR skill prompt 没镜像该例外 | `agents/independent-reviewer.md` — 加 self-referential review 模式分支：检测到目标 repo 是 iSparto 自身（CLAUDE.md 含 self-referential boundary 声明）时改 review docs/product-spec.md ↔ commands/ + agents/ + scripts/ 的实际行为对齐，不要求 docs/tech-spec.md；或在 `commands/end-isparto.md` Step 3 carve-out 旁加显式 "iSparto-as-self IR mode" 条款 | medium | 2026-05-12 v0.9.0 命令重命名 Wave — IR returned BLOCK on structural missing-tech-spec mismatch (假阳性 13 条 CRITICAL, 不是真 product↔technical 不对齐) |
| FR-48 | `agents/independent-reviewer.md` 的 write instruction (Step 9 / Step 3 fixed one-liner `Write your findings to docs/independent-review.md.`) 让 IR agent 直接 replace 整个文件而不是 append，每次 IR run 都把之前 Wave 的 IR 历史 wipe 掉；spec 字面说 "report appended" 但 prompt write-mode 是 replace。Live file 只剩当前 Wave 内容，历史 IR 只在 git 历史里能找回 | `agents/independent-reviewer.md` — 把 write 指令改为显式 append 行为（如 `Append your findings as a new top-level heading "## IR Review YYYY-MM-DD — <Wave name>" to docs/independent-review.md, do not overwrite prior content.`），或更新 F1 check 措辞接受 replace-mode 当 prior content git-recoverable | medium | 2026-05-12 PO audit of v0.9.0 命令重命名 Wave — F1 framework-side rule correction |
| FR-49 | PO audit canonical row count drift: `agents/process-observer-audit.md` 当前 19 行 (E series 含 E7 plan.md contract check)，但 plan.md Wave 4 historical entry (line 198) + 多处 docs/roles.md / docs/process-observer.md 仍说"canonical 18-row" — 是 FR-24 codify 后追加 E7 但 cross-references 没同步更新 | grep `18-row\|18 行\|"14 standard checks"` 全仓扫一遍 stale 引用，统一更新为 `19-row` / `19 行` (Tier 1/Tier 2 active files only — historical session-log + plan.md frozen Wave regions 保留旧措辞作为时间标记) | low | 2026-05-12 PO audit of v0.9.0 命令重命名 Wave — checklist row-count cross-reference drift |
| FR-50 | `docs/independent-review.md` 在 PR #244 被 IR replace 行为覆盖（855→73 行），848 行历史 IR audit 内容只在 git history 里能找回 | `git show bf97fd9:docs/independent-review.md > /tmp/ir.bak`；把当前 73 行 append 到末尾再写回；同时修 `agents/independent-reviewer.md` write 指令（已 FR-48）以防再次发生 | medium | 2026-05-12 Codex post-merge review of #244 |
| FR-53 | `docs/troubleshooting.md` 缺少跨机器 `~/.claude/settings.json` 同步最佳实践章节；用户实测发现自己把 `settings.json` symlink 到 iCloud Drive (`~/Library/Mobile Documents/com~apple~CloudDocs/claude-config/`) 实现多 Mac 共享配置，但这会触发 install.sh 绝对路径污染（已 FR-52 修）、iCloud 写冲突、机器特异性状态（MCP health cache / recent projects）跨机污染、与 Claude Code 设计假设（`~/.claude/` 应为机器本地）冲突 | `docs/troubleshooting.md` 新增 "Cross-machine settings.json sync" 章节：列出 iCloud-symlink 反模式 + 三种替代方案（A. 完全不 sync 各机独立装；B. dotfile-style git repo + 模板渲染工具如 chezmoi/yadm，把 `$HOME` 占位符在每台机器 render 时替换；C. 静态/动态配置分层 —— 当前 Claude Code 上游不支持，留作 future） | low | 2026-05-13 用户在 FR-52 close-out 讨论中自我识别 iCloud-symlink setup "可能不太对"，集体讨论收敛到 dotfile + chezmoi/yadm 模板渲染是业界共识做法 |
| FR-54 | `commands/end-isparto.md` Step 9 PR body template 是 Wave-scoped — `## Workflow audits` 三行 vocabulary（`Doc Engineer audit: [...]` / `Process Observer audit: [...]` / `Independent Reviewer: [...]`）只在 Wave-close PR 适用；非 Wave 的 governance-maintenance / multi-fix backlog-fix PR（如本 session 的 #249/#250/#251/#252/#253）走 ad-hoc-fix / 自指例外 / automated-release 路径时没有标准化注释模板，只能 inline prose 描述（如 "Codex review requirement waived"），导致 PO audit D3 检查 ambiguous（PO 只能从 prose 推断 audit decision，非 machine-checkable） | `commands/end-isparto.md` Step 9 — 在现有 Wave PR template 之外增加非 Wave variant，提供一行 `## Workflow audits: governance-maintenance — Lead self-assessed; DE/IR/PO: not triggered — reason: non-Wave backlog fix under self-referential boundary` 形式的 carve-out annotation，标准化 ad-hoc-fix / governance-maintenance / 自指例外的 audit 注释 vocabulary | low | 2026-05-13 PO audit of multi-fix + release session — D3 ambiguity observation |
| FR-55 | `commands/end-isparto.md` Step 8 C-layer logging rule 没规定 gh account auto-switch annotation 应该放在**当前** session 的 `### Notes` block 还是任何 session-log entry — PO audit 实测发现 2026-05-12 Session B 的 Notes 含 gh-switch 行但今天的 2026-05-13 session 的 Notes 没有对应 line，意味着 Step 8 logging 落点 ambiguous（之前 session 的 carryover？当前 session 必须新增？） | `commands/end-isparto.md` Step 8 — 在 C-layer logging 句子里加 "record in the current session's `### Notes` section" 限定，明确归属当前 session 而非 carryover | low | 2026-05-13 PO audit of multi-fix + release session — D4 C-layer placement ambiguity |
| FR-56 | In-repo deep links into `docs/session-log.md` headings (notably the plan.md "已完成 Wave 索引" table, ~28 rows) use single-hyphen anchor slugs, but GitHub renders headings containing em-dash / `+` / `/` separators as double-hyphen slugs (real: `#2026-04-20-session--wave-2--doctor-...`; written: `#2026-04-20-session-wave-2-doctor-...`) — the index links 404 on github.com. New README Evidence-section links were verified against rendered anchors and are correct; the legacy index table is not | sweep `docs/plan.md` index table + grep `session-log.md#` cross-references repo-wide; replace with rendered-verified slugs (extraction method: `curl <github blob URL> \| grep -oE '"anchor":"[^"]*"'`) | low | 2026-06-10 cross-provider repositioning session — discovered while link-verifying the new README Evidence section |
| FR-57 | `pre-tool-check.sh` push-on-main guard evaluates the session cwd repo's branch, not the branch of the repo a compound command `cd`s into — cross-repo sessions get false-positive blocks (observed: isparto-website feature-branch push blocked because the iSparto cwd repo sat on main); same path-blindness family as FR-40 | `pre-tool-check.sh` — resolve the target repo from a leading `cd <path> &&` or `git -C <path>` form and evaluate that repo's branch, or scope the push-on-main rule to commands targeting the cwd repo | low | 2026-06-10 cross-provider repositioning + CI session — observed during isparto-website PR #6 push |
| FR-58 | FR-19 carve-out condition (iii) is ambiguous for conversation-driven multi-PR sessions: "DE + PO audits both run as fresh sub-agent spawns in this invocation" — unclear whether "this invocation" means the `/end-isparto` close-out (whose Step 5/9 spawns WILL be fresh) or the whole session's per-PR audit coverage (which may be Lead-self-assessed), creating a catch-22 where the close-out's own fresh spawns cannot qualify the carve-out | `commands/end-isparto.md` Step 3 carve-out condition (iii) — define "this invocation" explicitly | medium | PO audit of 2026-06-10 Session — F1 FAIL analysis |
| FR-59 | No codified IR-skip annotation bucket for "blocked by structural agent defect" — Step 3 only defines the FR-19 carve-out shape and "IR ran"; a skip caused by a tracked framework defect (FR-47) has no machine-checkable vocabulary, forcing F1 into bare FAIL | `commands/end-isparto.md` Step 3 — add a third IR outcome annotation, e.g., a structural-defect-blocked bucket citing the FR number; mirror in the Step 9 PR template IR line | medium | PO audit of 2026-06-10 Session — F1 FAIL had no annotation home; may resolve together with FR-47 |
| FR-60 | `agents/process-observer-audit.md` F1 bucket conflates deliberate, documented, defect-blocked IR skips with oversight — no PASS-with-caveat / BLOCKED state | `agents/process-observer-audit.md` F1 row — add a distinct settled state tied to the FR-59 annotation vocabulary | low | PO audit of 2026-06-10 Session — F1 bucket precision gap |
| FR-61 | `agents/process-observer-audit.md` D3 row ("PR body template populated per end-isparto Step 9") lacks an explicit N/A clause for PRs generated by `scripts/release.sh` — auditors of release sessions must infer the exemption from `commands/release-isparto.md`'s "self-contained" language | `agents/process-observer-audit.md` D3 row — add one sentence: N/A also applies to release-script-generated PRs (CHANGELOG-body convention, cf. PRs #246/#254/#259) | low | PO audit of 2026-06-10 Session #b — v0.9.2 release |
| FR-62 | `scripts/gh-account-guard.sh` is only invoked for the iSparto repo by `/end-isparto` Step 9; cross-repo gh write operations have no guard touchpoint — the 2026-06-10 Vercel deployment-blocked incident (persisted account switch crossing into sister-repo merges, wrong-author merge commits) happened exactly where the guard never runs, despite the guard being repo-generic (derives owner from `git remote get-url origin`) | `commands/end-isparto.md` Step 8 (or the guard script header) — add a note that any cross-repo gh write op should run the guard from the target repo's cwd first | low | PO audit of 2026-06-10 Session #b — isparto-website Vercel incident |
...
| # | Item | Notes |
|---|------|-------|
| DV-1 | `/env-isparto` deep consistency checks (CLAUDE.md Module Boundaries ↔ actual disk structure, plan.md In-Progress ↔ branch diff) | Partial merit — CLAUDE.md-half higher value than plan.md-diff-half (latter has high false-positive risk on mid-task branches). Source: v0.7.8 Polish Wave Out of scope. |
...
| # | Item | Notes |
|---|------|-------|
| ED-1 | Heddle dogfooding — 3rd dogfooding scenario | Co-founded with Adam; generative UI runtime; will use `/init-isparto` from scratch. User-direction gate — iSparto framework side has no blocker. |
...
| ED-5 | main branch protection: add the 6 CI jobs as required status checks | The prepared `gh api -X PUT repos/BinaryHB0916/iSparto/branches/main/protection` call was denied by the Claude Code permission classifier mid-session (persistent security-config change beyond the approved CI scope); user runs it manually or re-authorizes in a future session. Check contexts: `bash syntax gate`, `shellcheck (warning severity)`, `guardian self-tests (macos-latest)`, `guardian self-tests (ubuntu-latest)`, `repo guardians (macOS / BSD userland)`, `installer smoke (macOS)`; preserve enforce_admins=true, review count 0, no force-push/deletions. Source: 2026-06-10 CI Wave close-out. |
...
| 2026-04-08 | Onboarding 提醒 | plan.md 挂"本地 hook 更新提醒"长期项,督促用户运行 `install.sh --upgrade` | 僵尸提醒（来自 2026-04-03,经过 10 个版本已失效）；时效性提醒不属于 plan.md,应该进 CHANGELOG + /start-isparto hook 自修复 | 未来需要用户重装 hook 时用 CHANGELOG + /start-isparto 提示替代 |
...
| 2026-04-17 | 框架 workflow | `/end-isparto` Step 5 让 PO audit 把 framework-side rule correction 写进 `docs/framework-feedback-MMDD.md` dated 文件（commands/end-isparto.md 67-71 行原指令） | 成了 plan.md 以外的并行 TODO 通道；0405–0417 共 12 天累计 11 个文件 21 条 open rule 绕过了用户"所有 TODO 进 plan.md"的 Single-TODO-source 意图 | 来源：2026-04-17 user-surfaced drift。永久 retired，由三条结构防御替换：CLAUDE.md Single-TODO-source Development Rule + commands/end-isparto.md 67-71 改写为"append 行到 plan.md Backlog" + Step 1 新增 TODO 归位 audit。PR #213 |
...
| 2026-06-10 | 产品定位 | 品类定为"跨模型编排框架"（cross-model orchestration framework） | 编排框架品类会落到 CrewAI/AutoGen/LangGraph 的 SDK 货架，在模型覆盖面/API 表面的对比上必输；"cross-provider AI dev team" 保留团队叙事（Spartoi 品牌 + v1.x/v2.x 演进载体），跨厂商降为头号属性。GPT-5.5 盲审同时否决完全删除 Claude Code 锚点（零外部用户期的唯一搜索锚） | 用户原方向经 GPT-5.5 盲审 + 讨论收敛到 Option B（PR #256）。触发条件：未来真做多 provider 适配层时重估"编排"品类 |
