...
| FR-26 | Doc Engineer audit does not mechanically re-execute plan.md acceptance grep patterns against the live codebase, so recorded "Actual" values can silently drift from ground truth (Wave 2 A6 pattern `^| /doctor |` recorded as `1 — PASS` but live re-execution returns `0` because the table cell uses backticks) | `docs/roles.md` Doc Engineer 10-item audit — add spot-check sub-item under item 3 (plan.md status update) requiring the auditor to re-execute at least one [code]/[build]-tagged acceptance row's command and diff exit code + output against the recorded "Actual" column | medium | PO audit of 2026-04-20 Wave 2 /doctor session — framework-side rule correction #3 |
| FR-27 | Observation Period tracker placeholder cells (`待填 — ... 后填充`) have no explicit resolution-timing rule — can ship in a commit unresolved, violating audit-trail integrity | CLAUDE.md Development Rules or `commands/end-working.md` Step 2 — add one-line rule: "tracker placeholder cells under `docs/plan.md` §v0.8.0 升级观察期 Tracker must be filled before the Wave commit lands (audited as part of `/end-working` Step 9 pre-merge DE audit item 3)" | low | PO audit of 2026-04-20 Wave 2 /doctor session — framework-side rule correction #4 |
