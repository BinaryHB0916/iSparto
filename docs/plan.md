...
| FR-35 | `docs/zh/quick-start.md` 停留在 v0.7 前的认知: tmux 描述为 iTerm2 内置集成 (与 v0.8.0 hard dependency 冲突), 命令覆盖只有 /init-project /migrate /start-working /end-working /restore 五个 (缺 /plan /env-nogo /doctor /security-audit), 无 IR via Codex / plan.md Backlog / A/B/C 信息分层等 v0.8.0 语境 | `docs/zh/quick-start.md` — 重写 "系统要求" 段 tmux 条目为 v0.8.0 hard dep, "日常工作流" 段补 /plan /env-nogo /doctor /security-audit 四命令说明, 加入 IR-via-Codex 一句 + plan.md Backlog 作为 TODO 单一来源的 pointer | medium | v0.8.0 doc alignment Plan Phase 2 deferred — 2026-04-24 plan at `/Users/duanshao/.claude/plans/joyful-hatching-sky.md` |
...
| FR-38 | `agents/process-observer-audit.md` E6 row precondition text references "plan.md around line 963" for the observation-period tracker heading location, but post-reshape the tracker heading `### v0.8.0 升级观察期 Tracker` now lives at plan.md line 178 — the approximate line-number pointer is stale and misleads anyone manually navigating to verify the precondition (the heading-text grep remains operative, so no logic defect, only a stale navigation aid) | `agents/process-observer-audit.md` E6 row — replace "around line 963" with "search for the heading `### v0.8.0 升级观察期 Tracker` in plan.md" or remove the approximate line number since the heading text is the reliable anchor | low | PO audit of 2026-04-24 Session — Wave v0.8.0 Doc Alignment Phase 1 Factual Corrections |
