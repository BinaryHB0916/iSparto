...
| FR-51 | `install.sh:48` `version_lt()` 用 `sort -V`，macOS BSD sort 便携性风险（pre-Big Sur 不支持，Big Sur+ 看版本） | 改纯 bash field-by-field semver 比较，或确认 `install.sh` 显式只支持的最小 macOS 版本并文档化 | low | 2026-05-12 Codex post-merge review of #244 |
| FR-52 | `lib/patch-settings.py` 在 patch `~/.claude/settings.json` 的 hook command 字段时把 `$HOME` 在 install 时立刻展开成绝对路径（如 `bash /Users/duanshao/.isparto/hooks/process-observer/scripts/pre-tool-check.sh`），跨机器（不同 macOS 用户名）装机后会累积多条等价但路径不同的 hook 条目；patch 时仅按 command 字符串精确匹配去重，绝对路径不同即视为新条目；Claude Code 把每条 hook 当 non-blocking 执行，破损条目（指向不存在的用户目录）持续报 `No such file or directory` 但不阻断工具调用，造成长期噪音和审计混淆 | (a) `lib/patch-settings.py` 改为写入 `$HOME` 字面量并包裹在 `if [ -f "$HOME/.isparto/.../pre-tool-check.sh" ]; then bash "..."; fi` 形式（本机 `~/.claude/settings.json` 已有一条该形式条目，说明某个早期版本曾这么写过——找到 regression 引入点确认治理范围）；(b) patch 时按 hook 脚本尾缀（`.isparto/hooks/process-observer/scripts/pre-tool-check.sh`）等价去重，绝对路径变化的等价条目视为重复并清理；(c) `install.sh --upgrade` 增加一次性 migration 清除已存在的绝对路径破损条目（file-existence gated，幂等） | major | 2026-05-13 用户在 PR #250 收口时反馈：本机 `~/.claude/settings.json` 实测累积 3 条等价 pre-tool-check.sh hook 条目（1 条 `$HOME` 字面量可移植、1 条 `/Users/duanshao/...` 破损、1 条 `/Users/duanwei/...` 重复但可用），跨 Mac Mini ↔ 本机装机模式触发 |
