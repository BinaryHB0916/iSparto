...
- **plan.md verification-count accuracy:** when a Wave completion entry records a commit count, compute it mechanically via `git log --oneline --no-merges <wave-base>..HEAD | wc -l` at the time the entry is written — not by estimation. Applies to every Wave close-out. The `<wave-base>` is the commit where the Wave branch diverged from the target branch.
...
- **Tier 4 — Historical Artifacts (frozen):** `docs/session-log.md`, `docs/framework-feedback-*.md`, `docs/plan.md` (wholly Tier 4 — both historical Wave entries and forward-looking sections such as the Next-Steps roadmap and Tech Ecosystem Tracking table are exempt from the English-only rule; new plan.md entries default to English but may mix in CJK where a user conversation would lose nuance from translation), and historical entries in `CHANGELOG.md`. Tier 4 files are not modified retroactively; new `CHANGELOG.md` entries use English. The `scripts/language-check.sh` guardian's Tier 2 exclusion list reflects this (see `TIER2_EXCLUDED_FILES` in the script).
