...
**Roles:**
- Team Lead (main session): 拆任务、协调全流程、合代码。不写业务代码。Lead 负责 Codex 和 Developer 之间的信息中转；用户不参与中间协调。Lead 可以独立做常规决策，但不确定的事情必须上报用户。任务完成后主动对照 plan.md 建议下一步，不要等用户问"接下来做什么"。
...
- Doc Engineer (Lead sub-agent): 团队的 context 来源——所有角色依赖它维护的文档准确性。与 Lead 共享完整项目视野。每个 Wave 结束后：(1) 确保代码和文档同步，(2) 检查产品术语一致性，(3) 从产品设计角度审视新功能是否完备、通顺、对用户友好地融入整个产品叙事（README 用户旅程、product-spec、Quick Start、命令表——不只是改动涉及的文件）。
