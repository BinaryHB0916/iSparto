...
| **tmux teammate 模式** | Claude Code 内置的 Agent Team 运行方式。多个 Developer 在各自的 tmux pane 中并行工作，由 Lead 协调。需要安装 tmux（`brew install tmux`），但不需要你手动配置——Claude Code 自动管理会话。macOS 推荐 iTerm2（原生 tmux 集成体验更好）。 | 类似多个终端窗口同时工作 |
...
| Git | 任意版本 | 版本控制 |
| tmux | 必须 | Agent Team 每个 teammate 在独立 pane 中并行工作，用户可实时看到所有 agent 的进度。`brew install tmux` |
| 终端 | iTerm2 推荐 | macOS 推荐 iTerm2（原生 tmux 集成体验更好）；其他终端也可用 |
...
# 6. 在你的项目里添加 Codex MCP Server（每个项目执行一次）
cd your-project/
claude mcp add codex-reviewer -s project -- npx -y codex-mcp-server

# 7. 重启 Claude Code，验证 MCP 连接
...
| Agent Team 启动失败或 teammate 不可见 | tmux 未安装 | `brew install tmux` 后重启 Claude Code |
...
- [ ] tmux 已安装（`brew install tmux && tmux -V` 验证）
- [ ] 终端推荐 iTerm2（macOS，tmux 集成体验更好）
