...
## See Also — A-layer Peer Review

A-layer Peer Review (formerly Mode 3 of this role, added in v0.7.4) has been extracted to `docs/design-principles/a-layer-peer-review.md`. When Lead's spawn message includes the A-layer Peer Review fixed prompt (defined in that file), follow the new file for the invocation trigger, tool permissions (read-only by default, with deep-IR gate), four judgment axes, verdict format, conflict-resolution rule, and scope boundaries. The A-layer review operates on a different set of primary sources (the proposed A-layer text + Policy classification rules + conversation-style wording rules) than the Phase 0 / Wave Boundary modes defined above.
