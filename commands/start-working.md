...
Your responsibility: Report current status and wait for user confirmation before launching the team. Do not write any code.
...
3. Quick check: Is the current code state consistent with docs/ documentation, or has any drift occurred?
4. Confirm the current branch (should be on a feat/, fix/, or hotfix/ branch — do not develop on main)
5. Determine the next collaboration mode and suggest to the user:
...
6. Present all the above information and wait for the user to confirm "start" before launching the team per the Lead workflow
