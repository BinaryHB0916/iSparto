...
0.5. Branch guard (before reading any files or doing any work):
   ...
   - If on `main` or `master`: immediately run `git checkout -b feat/wip-MMDD` (where MMDD is today's date) to create a working branch
     - C-layer: silently log the auto-create to session-log at /end-isparto time. Do NOT narrate it in the Step 9 briefing unless it materially affects the user's next decision (it rarely does — the user will see the branch name in the briefing's B-layer state-variable sentence anyway).
...
6. GitHub account alignment (if applicable):
   ...
   - If both are non-empty and REPO_OWNER ≠ GH_USER:
     ...
     - C-layer: auto-switch is silent. The framework handles account alignment autonomously; the user does not need to be told. Log to session-log at /end-isparto time if switching occurred.
   - If gh is not installed, not authenticated, or only one account exists: skip silently
   - This step ensures `gh pr create` in /end-isparto uses the correct account
