...
3. Wave Boundary Review (conditional):
   ...
   - **Skip carve-out (self-referential polish Wave).** When the Wave-completion trigger fires, the Lead MAY skip sub-steps a–d below if ALL three conditions hold — each condition is independent; missing any one reverts the Wave to the default "run IR" path:
     ...
     (iii) **DE + PO audits both run as fresh sub-agent spawns in this invocation** — Doc Engineer audit (Step 9 pre-merge gate) and Process Observer audit (Step 5) both execute as fresh sub-agent spawns in this `/end-isparto` invocation, i.e., the Wave is NOT operating under the ad-hoc-fix or emergency-hotfix exceptions from CLAUDE.md Solo/Agent Team workflow step 4. Those exceptions skip one or both audits and would leave the Wave with no guardian coverage if IR is also skipped; condition (iii) exists to prevent that dead-zone.
...
4. Generate a session report and append it to `docs/session-log.md`:
   ...
   - **plan.md, session-log.md and CHANGELOG authoring + transition contract.** Before committing, the Lead applies this contract to every file touched by this `/end-isparto`. The contract has three parts:
     - **A. Authoring rule (content ownership).**
       ...
       - `CHANGELOG.md` — positioning: **user-facing release notes in Keep-a-Changelog format.** `[Unreleased]` collects pre-release content; `scripts/release.sh` sed-migrates `[Unreleased]` to `[X.Y.Z]` at `/release-isparto` time. Only user-facing changes land here — internal refactors that produce no user-observable delta do not.
     ...
     - **B. Transition rule (the behavioural fix that each `/end-isparto` must run).** At each `/end-isparto` triggered by a Wave / FR / governance-maintenance completion:
     ...
     - **C. Enforcement.** Each `/end-isparto` runs `bash scripts/plan-md-contract-check.sh` before the commit lands. Non-zero exit blocks the commit. The Doc Engineer audit item 11 then does semantic verification beyond what the script catches mechanically (transition-completeness, link-vs-copy, narrative-home correctness).
5. Spawn the "Process Observer Audit" agent (Sonnet model) to audit this session:
   ...
   - If rule correction suggestions are identified, record them in the briefing for the next /start-isparto session to reference (they become next-session carry-over, not this-session narration)
...
7. Branch guard before commit:
   ...
   - If on main and there are uncommitted changes (session log, docs updates, etc.):
     ...
     - This happens when the main work was already merged via PR before /end-isparto ran
...
9. If all tasks on the current branch are complete (all reviews passed, docs updated):
   ...
   - If the third re-audit still FAILs (loop bound exceeded), execute the **6-step blocked recovery path** defined in `docs/roles.md` Doc Engineer Key Principles: (1) stop the loop; (2) generate a blocked-audit report capturing the final FAIL state; (3) write a blocked-audit entry to `docs/plan.md` under a "Blocked audits" section; (4) `git push -u origin <current-branch>` to preserve WIP; (5) report to the user (in user's language) that Doc Engineer audit hit the loop bound, the WIP branch is pushed, and manual intervention is required; (6) exit `/end-isparto` without creating or merging a PR. Do not leave recovery to Lead's improvisation.
   - **gh account mid-session guard (FR-13, immediately before `gh pr create`).** Step 8's alignment snapshot can go stale between Step 8 and the PR call if the user manually ran `gh auth switch` mid-session or if the re-audit loop above spanned long enough for an external tool to flip the active account. Run `bash scripts/gh-account-guard.sh`:
     - Exit 0 (aligned, auto-recovered, or no gh data to check): proceed silently to the PR-creation bullet below. C-layer: guard-pass is not narrated.
     - Exit 2 (mismatch persists after the guard's own `gh auth switch` attempt): **A-layer (Policy trigger type e — critical intercept).** Do NOT run `gh pr create`. Emit one A-layer interrupt using the standard wording rule: name the drift (target REPO_OWNER vs. live GH_USER from the guard's stderr), state that the Lead is NOT creating the PR because the PR would be attributed to the wrong account, propose the concrete remediation (user manually runs `gh auth login --hostname github.com --web` to re-add the target account, or runs `gh auth switch --user <REPO_OWNER>` and confirms, then re-invokes `/end-isparto`). Wait for user response before resuming.
     - If `scripts/gh-account-guard.sh` is not present on disk (stale local install predating this step): skip the guard silently and proceed — the Step 8 snapshot is the best available signal. The next `install.sh --upgrade` will land the guard for subsequent sessions.
...
1. **What shipped today** — one sentence naming the concrete outcome, referencing Wave completion if applicable (e.g., "Wave 7 / v0.7.4 Information Layering Policy is complete — T1–T5 merged to main via PR #NNN"). If mid-Wave, name the tasks that closed (e.g., "T1 and T2 of Wave 7 are done; T3 is next"). This is the cross-session recovery surface for the next /start-isparto — it must always be emitted.
...
3. **What's next** — one sentence proposing the next concrete action using the A-layer wording rule only when there IS a next decision the user needs to make; otherwise a plain pointer ("next: T3 at the next /start-isparto" or "Wave is complete, awaiting your direction"). If a Wave Boundary Review CRITICAL was raised in Step 3, this sentence names the CRITICAL as the blocker to the next Wave.
