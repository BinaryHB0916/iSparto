...
- **plan.md update cadence:** update `docs/plan.md` either per-task (in the same commit as the task work) OR per-Wave (in a close-out commit that lists all task completions with commit hashes — acceptable when the Wave runs as a single atomic work session on a dedicated branch). Wave-completion entries and cross-session BLOCKING markers are written by `/end-isparto` as part of the commit it generates. If a fix session does not correspond to any plan.md entry (e.g., a bug fix not tied to any Wave), no plan.md update is required.
- **plan.md verification-count accuracy:** when a Wave completion entry records a commit count, compute it mechanically via `git log --oneline --no-merges <wave-base>..HEAD | wc -l` — not by estimation. For entries authored pre-commit (standard `/end-isparto` cadence), write the projected count and re-verify via the same command immediately after the commit lands; if mismatch, amend before push.
...
- Developer (Codex MCP): Implements code per structured prompts from Lead/Teammate. Also handles QA smoke testing (different prompt + different model, selected by Lead based on Tier). Two-tier models: implementation uses gpt-5.5 (xhigh), QA/quick fixes use gpt-5.5-mini (high). See docs/configuration.md.
...
3.5 (Phase 0 only, or Wave completed) Lead spawns Independent Reviewer in a tmux pane via `codex exec` (fixed one-liner), waits for report. CRITICAL finding → stop development, resolve alignment first; after CRITICAL fix, must re-trigger
...
- Use /restore-isparto to roll back if /init-isparto or /migrate-isparto produces unexpected results
...
**Runtime output layering:** see `docs/design-principles/information-layering-policy.md`. Lead must classify every user-facing output as A-layer (decision interruption), B-layer (decision preparation at natural pause points: `/start-isparto` open, `/end-isparto` close, `/plan-isparto` proposal), or C-layer (silent archive) before emitting it. The Policy is enforced structurally via fixed B-layer briefing shapes in `commands/start-isparto.md`, `commands/end-isparto.md`, and `commands/plan-isparto.md`; Lead's runtime judgment narrows to word choice, not layer assignment. A-layer outputs are subject to Independent Reviewer A-layer Peer Review (see `docs/design-principles/a-layer-peer-review.md`).
...
- Session log -> docs/session-log.md (auto-generated by /end-isparto)
...
- Information Layering Policy -> docs/design-principles/information-layering-policy.md
  <!-- Governs how Lead classifies user-facing outputs (A-layer decision interruption / B-layer briefing / C-layer silent) -->
- Conversation style guide -> docs/design-principles/conversation-style.md
  <!-- Concrete wording rules and before-after samples for A-layer and B-layer outputs -->
