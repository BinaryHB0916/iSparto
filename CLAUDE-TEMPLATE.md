...
- Developer (Codex MCP): Implements code per structured prompts from Lead/Teammate. Also handles QA smoke testing (different prompt + different model, selected by Lead based on Tier). Two-tier models: implementation uses gpt-5.4 (xhigh), QA/quick fixes use gpt-5.4-mini (high). See docs/configuration.md.
